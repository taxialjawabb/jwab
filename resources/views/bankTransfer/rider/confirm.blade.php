...
<h5 class="mt-4">تأكيد حفظ سند للعميل : {{ $d['name'] }}</h5>

<form  method="POST" action="{{ url('bank/transfer/rider/add') }}">
    @csrf
    <input type="hidden" id="rider_id" name="rider_id" value="{{ $d['rider_id'] }}" required>
...
<div class="text-center image m-3">
    <img src="{{ asset('assets/images/riders/banktransfer/'.$d['transfer_photo'])}}" style="width: 500px; height: 500px" id="profile-img-tag" alt="صورة العميل">
</div>
