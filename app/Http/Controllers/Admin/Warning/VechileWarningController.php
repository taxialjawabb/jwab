...
class VechileWarningController extends Controller
{
    public function show($type)
    {
        ...
        if($type === 'driving_license_expiration_date'){
            ...
            DATEDIFF(driving_license_expiration_date , now()) as days from vechile where DATEDIFF(driving_license_expiration_date , now()) < 5  order by  days;");
            ...
            DATEDIFF(insurance_card_expiration_date , now()) as days from vechile where DATEDIFF(insurance_card_expiration_date , now()) < 5  order by  days;");
            return view('warning.vechileWarning', compact('vechiles', 'type'));
        }
            ...
            DATEDIFF(periodic_examination_expiration_date , now()) as days from vechile where DATEDIFF(periodic_examination_expiration_date , now()) < 5  order by  days;");
            return view('warning.vechileWarning', compact('vechiles', 'type'));
        }
            ...
            DATEDIFF(operating_card_expiry_date , now()) as days from vechile where DATEDIFF(operating_card_expiry_date , now()) < 5  order by  days;");
            return view('warning.vechileWarning', compact('vechiles', 'type'));
        }
        else{
            return back();
